...
**Javier 2026-09-09:** el bloque `#vip` no muestra precio (`1,99 €/mes`) ni «Cancelar suscripción» en ningún entorno (tampoco PRE, aunque exista checkout URL). Re-activar: `SHOW_VIP_PRICE_AND_CANCEL = true` en `components/vip-promo.tsx` cuando Javier lo autorice. No inventar un precio nuevo.

En **PRE / preview / local** el bloque `#vip` de `/correr` y `/ciclismo` muestra qué incluye (aviso la víspera, aviso el mismo día por Telegram VIP privado, push y correo) y cupo 100. El CTA puede ser un enlace activo («Quiero el Canal VIP») que abre el Stripe **test** Payment Link en una pestaña nueva. Success/cancel se configuran en Stripe; la web no monta checkout embebido. **No hay enlace `t.me` ni invite permanente de Telegram.** En **producción (`VERCEL_ENV=production`)** el botón es siempre interés («Próximamente»): `getVipCheckoutUrl()` devuelve `null` aunque existan Payment Link, `VIP_CHECKOUT_URL` o `NEXT_PUBLIC_VIP_CHECKOUT_ENABLED`. No setear esas variables en el entorno Production de Vercel.

La ruta `/vip/cancelar` sigue existiendo (PRE / test; no indexar) pero **no se enlaza desde `#vip`** mientras `SHOW_VIP_PRICE_AND_CANCEL` esté en `false`. Cuando se reactive, «Cancelar suscripción» irá a `/vip/cancelar` (o a `NEXT_PUBLIC_STRIPE_CUSTOMER_PORTAL_URL` / `VIP_CUSTOMER_PORTAL_URL` si está seteada). `/vip/cancelar` envía el email a `POST /api/vip/portal`, que busca el customer en Stripe test (`customer=cus_…` o email) y crea una Billing Portal Session. `return_url` vuelve al origen PRE (`/`). Si el portal de test no está activado en el Dashboard, la página muestra «Portal no activado aún». Si no hay customer, «necesitas una suscripción activa». No se inventan customers ni URLs de portal.
